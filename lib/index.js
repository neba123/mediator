...
function setupApp () {
  ...
  app.all('/encounter', async (req, res) => {
    ...
    var fetchURL= 'https://34.122.219.240/openmrs/ws/atomfeed/encounter/recent';
    ...
    for(var v=0; v < content.length;v++){
      ...
      let encounterURL= "https://34.122.219.240"+ content[v];
      ...
      if(encounterType == "Consultation"){ 
        ...
        //patient information
        let patientURL= "https://34.122.219.240/openmrs/ws/rest/v1/patient/"+ encounterDetail.data.patientUuid + '?v=full';
        ...
        try{
          server.send('192.168.0.145', 15000 , order_all, await function (err, ackData) {         
...
app.post('/labresult', async (req, res) => {
    ...
    try{
      ...
      let result_orc = '"accessionUuid": "' + orc_array[indexes.accessionUuid] + 
                       '", "patientUuid": "' + orc_array[indexes.patientUuid] + '", "locationUuid": "' + orc_array[indexes.locationUuid] +
                       '", "encounterUuid": "' + orc_array[indexes.encounterUuid]  + '"';

     accessionUuID= orc_array[indexes.accessionUuid];
     //OBX
     ...
     for(var i=0; i< testDetails.length; i++){
       ...
       if(testResults == '{') {
          testResults += '"testName": "' + test_name[0] +
                  ...
                  '", "abnormal": "' + obx_array[indexes.abnormal] +
                  '", "status": "' + obx_array[indexes.rsltStatus] +
